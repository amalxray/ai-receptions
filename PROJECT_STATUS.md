...
### ✅ مُصلَحة (2026-09-21 — الإصلاح الثنائي 29 + 34)
- **#29 تغيير كلمة المرور لا يطلب الحالية** → صفحة الملف الشخصي تطلب «كلمة المرور الحالية» وتتحقق منها عبر `signInWithPassword` قبل `updateUser` (إثبات هوية + جلسة جديدة تمنع «Auth session missing»).
- **#34 إضافة إنجاز → Internal error** → الجذر: بوابة الميزات ترد `402 FEATURE_LOCKED` (الشارات لخطة center فقط — hala-clinic أصبحت unpaid) وكانت الواجهة تعرضه كنص خام مع بقاء النموذج، وأخطاء 500 تخفي السبب الحقيقي. الآن: صفحة الشارات تعرض لوحة «ميزة مقفلة» مع زر ترقية بدل النموذج، وكل استجابات 500 تُظهر `detail` بالسبب الفعلي (badges + public-content + [badgeId])، وأنهيت TODO التشخيصي المؤقت. (منح `service_role` على جداول المحتوى مُتحقَّق منها وتعمل — مهاجرة 20261004.)

### ✅ مُصلَحة (2026-09-21 — إصلاحات الموبايل 39 + 40)
- **#39 الحجز على الهاتف** → اختبار حقيقي بمتصفح مُحاكي (iPhone 375×812, touch) على `/book`: الخدمة → الطبيب → **تقويم مخصّص** بدل `<input type="date">` (لم يكن يعرض أي placeholder ولا يفتح المنتقي على iOS Chrome/Safari) → لمس اليوم ينقل لخطوة الوقت ويحفظ التاريخ. الحالة تُحفظ في الرابط (`step/service/provider/date/time` بلا تنقل) فإعادة تحميل Safari للتبويب لا تُفقد الاختيار، وزر «↩ تغيير التاريخ» يعود والتاريخ باقٍ مظلَّل.
- **#40 Mobile UX** → `viewport-fit=cover` + `maximumScale=5`، شعار الصفحة العامة `h-16 sm:h-24`، إحصائيات الـ Hero عمود واحد على الموبايل، قائمة لوحة التحكم hamburger (drawer RTL) بدل 27 وحدة مكدّسة، جدول Leads داخل `overflow-x-auto`، أهداف اللمس ≥44px و`touch-action: manipulation` في كل خطوات الحجز.
- **إثر #39: تقويم الحجز المخصّص** → `components/booking/DatePicker.tsx` (react-day-picker v10 + عربي/RTL + سبت أول الأسبوع) مع `lib/booking/calendar.ts` (تحويلات ISO آمنة زمنيًا + قاعدة قبول اليوم) و`GET /api/booking/calendar` الذي يُغلق أيام عطل العيادة (`clinic_holidays`) وإجازات الطبيب (`provider_vacations`) وأيام عدم العمل الأسبوعية — **fail-open**: أي فشل في الفحص لا يحجب أي يوم، و`/api/booking/availability` يبقى الحاكم لما هو قابل للحجز فعلاً، وإن رجع يوم بلا أوقات تظهر ملاحظة على التقويم نفسه («لا توجد أوقات متاحة في … — جرّب يومًا آخر»). التحقق: `npm run verify:booking` — **16/16** على 320/375/414px (iPhone touch) و1280px (سطح المكتب).
...
1. ~~إصلاح الأخطاء 29 + 34~~ ✅ تم (2026-09-21 — الإصلاح الثنائي)
