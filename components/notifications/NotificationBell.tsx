...
export type InAppNotification = {
  ...
  payload: {
    ...
    body?: string;
    link?: string;
    /** B20 — the referral event name carried inside the payload. */
    event?: string;
  };
  created_at: string;
};
...
const TYPE_ICONS: Record<string, { icon: string; bg: string }> = {
  ...
  // B20 — cross-tenant referral lifecycle (payload.event, written by referralNotifier)
  referral_submitted: { icon: '🩻', bg: 'bg-cyan-500/10 text-cyan-400 border-cyan-500/30' },
  referral_accepted: { icon: '✅', bg: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' },
  referral_rejected: { icon: '❌', bg: 'bg-rose-500/10 text-rose-400 border-rose-500/30' },
  referral_needs_clarification: { icon: '❓', bg: 'bg-amber-500/10 text-amber-400 border-amber-500/30' },
  referral_result_ready: { icon: '📤', bg: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' },
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-slate-800/60">
            {loading && notifications.length === 0 ? (
              ...
              notifications.map((n) => {
                ...
                // B20 — referral notifications are stored with type='system' and
                // their event inside the payload, so the icon resolves from the
                // EVENT first (then the row type, then the generic fallback).
                const style =
                  TYPE_ICONS[n.payload?.event ?? ''] || TYPE_ICONS[n.type] || TYPE_ICONS.system;
                return (
                  <div
                    key={n.id}
                    onClick={() => {
